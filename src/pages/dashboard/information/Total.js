import sumBy from 'lodash/sumBy';
import { useState } from 'react';
// @mui
import { Grid, Stack, Divider, Container } from '@mui/material';
import { useTheme } from '@mui/material/styles';
import Scrollbar from 'src/components/Scrollbar';
import PassDay from 'src/sections/@dashboard/total/PassDay';
import GamesOverview from 'src/sections/@dashboard/total/GamesOverview';
import GamesTotalScore from 'src/sections/@dashboard/total/GamesTotalScore';

import Page from '../../../components/Page';

export default function GeneralTotal() {
  const theme = useTheme();

  return (
    <Page title="Dashboard | Point">
      <Container maxWidth="xl">
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <Stack direction={{ xs: 'row', md: 'column', lg: 'column' }} spacing={3}>
              <PassDay
                title="AttnKare-T 시작한지"
                icon={'eva:diagonal-arrow-left-down-fill'}
                percent={52.8}
                total="58"
                chartData={[1, 3, 5, 7, 5]}
              />
            </Stack>
          </Grid>
          <Grid item xs={12} md={12}>
            <Scrollbar>
              <Stack
                direction="row"
                divider={<Divider orientation="vertical" flexItem sx={{ borderStyle: 'dashed' }} />}
                sx={{ py: 2 }}
              >
                <GamesOverview
                  title="탭"
                  times={12}
                  total={54}
                  percent={60}
                  icon="eva:checkmark-circle-2-fill"
                  color={theme.palette.success.main}
                />
                <GamesOverview
                  title="비즈"
                  times={6}
                  total={23}
                  percent={34}
                  icon="eva:bell-fill"
                  color={theme.palette.error.main}
                />
                <GamesOverview
                  title="방정리"
                  times={9}
                  total={32}
                  percent={57}
                  icon="eva:clock-fill"
                  color={theme.palette.warning.main}
                />
                <GamesOverview
                  title="의미카드"
                  times={11}
                  total={40}
                  percent={68}
                  icon="eva:checkmark-circle-2-fill"
                  color={theme.palette.success.main}
                />
                <GamesOverview
                  title="레이싱"
                  times={30}
                  total={89}
                  percent={100}
                  icon="eva:checkmark-circle-2-fill"
                  color={theme.palette.success.main}
                />
              </Stack>
            </Scrollbar>
          </Grid>
          <Grid item xs={12} md={6} lg={4}>
            <GamesTotalScore
              title="탭 종합 점수"
              index="tab"
              subheader="(+14%) than last week"
              chartLabels={['1주차', '2주차', '3주차', '4주차', '5주차', '6주차', '7주차', '8주차']}
              chartData={[
                {
                  data: [
                    { name: '종합점수', data: [60, 62, 65, 63, 67, 72, 77, 78] },
                    { name: '집중력', data: [14, 5, 15, 45, 46, 26, 47, 50] },
                    { name: '작업기억력', data: [5, 7, 10, 12, 15, 12, 23, 20] },
                    { name: '충동성', data: [50, 45, 33, 30, 25, 30, 35, 36] },
                  ],
                },
              ]}
            />
          </Grid>
          <Grid item xs={12} md={6} lg={4}>
            <GamesTotalScore
              title="비즈 종합 점수"
              index="biz"
              subheader="(+14%) than last week"
              chartLabels={['1주차', '2주차', '3주차', '4주차', '5주차', '6주차', '7주차', '8주차']}
              chartData={[
                {
                  data: [
                    { name: '종합점수', data: [45, 47, 49, 52, 43, 44, 45, 48] },
                    { name: '집중력', data: [30, 31, 35, 37, 40, 43, 45, 47] },
                    { name: '행동조절', data: [5, 7, 10, 12, 15, 12, 23, 20] },
                  ],
                },
              ]}
            />
          </Grid>
          <Grid item xs={12} md={6} lg={4}>
            <GamesTotalScore
              title="방정리 종합 점수"
              index="room"
              subheader="(+14%) than last week"
              chartLabels={['1주차', '2주차', '3주차', '4주차', '5주차', '6주차', '7주차', '8주차']}
              chartData={[
                {
                  data: [
                    { name: '종합점수', data: [78, 79, 80, 82, 82, 83, 80, 80] },
                    { name: '집중력', data: [30, 32, 36, 40, 46, 50, 52, 54] },
                    { name: '집행기능', data: [35, 45, 50, 50, 52, 53, 54, 54] },
                  ],
                },
              ]}
            />
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <GamesTotalScore
              title="의미카드 종합 점수"
              index="card"
              subheader="(+14%) than last week"
              chartLabels={['1주차', '2주차', '3주차', '4주차', '5주차', '6주차', '7주차', '8주차']}
              chartData={[
                {
                  data: [
                    { name: '종합점수', data: [56, 57, 58, 58, 57, 60, 62, 62] },
                    { name: '집중력', data: [30, 32, 36, 40, 46, 50, 52, 54] },
                    { name: '충동성', data: [50, 45, 45, 45, 35, 30, 33, 36] },
                  ],
                },
              ]}
            />
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <GamesTotalScore
              title="레이싱 종합 점수"
              index="racing"
              subheader="(+14%) than last week"
              chartLabels={['1주차', '2주차', '3주차', '4주차', '5주차', '6주차', '7주차', '8주차']}
              chartData={[
                {
                  data: [
                    { name: '종합점수', data: [60, 61, 62, 63, 65, 67, 70, 72] },
                    { name: '집중력', data: [50, 52, 56, 50, 56, 58, 52, 70] },
                    { name: '작업기억력', data: [20, 22, 24, 25, 26, 27, 27, 28] },
                    { name: '집행기능', data: [33, 35, 35, 36, 36, 37, 38, 40] },
                    { name: '행동조절', data: [5, 12, 10, 12, 15, 12, 23, 20] },
                  ],
                },
              ]}
            />
          </Grid>
        </Grid>
      </Container>
    </Page>
  );
}
